...
$(function () {
    ...
    function renderProperties(node /* ViewNode */) {
        ...
        const addProp = function (p, type) {
            ...
            const propertyName = labelProtoType.cloneNode()
            propertyName.appendChild(spanProtoType.cloneNode())
            propertyName.appendChild(document.createTextNode(p.name))
            nameSubContainer.appendChild(propertyName)
            ...
            if (value == "") {
                ...
                if (!isNaN(valueF)) {
                    ...
                    if (valueF == valueI) {
                        ...
                        if (valueHex) {
                            colorWell = divProtoType.cloneNode()
                            colorWell.classList.add(CLS_COLORWELL)

                            selectTag.onchange = () => {
                                const myVal = "" + selectTag.val();
                                if (myVal.startsWith("#")) {
                                    const webColor = '#' + toHex(argb2rgba(valueU), 8);
                                    colorWell.style.display = 'inline-block'
                                    colorWell.style.backgroundColor = webColor
                                } else {
                    ...
                    if (colorWell) labelTag.appendChild(colorWell);
                } else {
            ...
            const starContainer = spanProtoType.cloneNode()
            starContainer.classList.add("star")
            starContainer.pName = p.fullname
            propertyName.insertBefore(starContainer, null)
            starContainer.onclick = toggleFavorite
    ...
    tlHvAction = function(appInfo) {
        ...
        function migrateSelectedState(index /* Integer */) {
            ...
            if (hasDifferentProperties(lastFramesSelectedNode, selectedNode)) {
                /* renderProperties currently recreates and redraws the entire properties
                   window whenever 1 item has changed. If view hierarchy switch latency
                   ever becomes an issue, we can optimize by only updating the text within
                   the 'select/option' html elements in the properties window right pane
                   since the attributes for all time lapse nodes are the same. */ 
